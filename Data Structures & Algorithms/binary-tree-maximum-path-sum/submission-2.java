...
class Solution {
    ...
    public int maxPathSumx(TreeNode root) { 
        ...
        int left = maxPathSumx(root.left);
        int right = maxPathSumx(root.right);

        int value = Math.max(Math.max(Math.max(val, val+left), val+right), val+left+right); 
        max = Math.max(max, value);
        System.out.println(value);
        return Math.max(Math.max(val, val+left), val+right);
    }
}
