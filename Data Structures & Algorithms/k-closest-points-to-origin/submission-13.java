...
public class Solution {
    ...
    private int fix_pivot(int l, int r) {
        ...
        for(int j=l; j<=r; j++) {
            ...
            if(j!=r) i++; // do not increment i if the loop reached r. we need the index of fixed last element only. if we not check this condition then return --i. return i-- is wrong as i is passed to return stmt and then only decrement happens in i--
        }
        return i;
    }
