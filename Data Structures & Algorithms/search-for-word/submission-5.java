...
class Solution_backtracking_visited_1D_array {
    public boolean exist(char[][] board, String word) {
        if(board == null) {
            if(word == null) return true;
            return false;
...
// same timecomplexity as backtracking hashset O(m * 4^n)
// recurrsion stack space same O(n), visited array O(m), O(1) for output
// where m is number of cells in board, n is length of word

// ---------------------------------------------------------------------

// Solution 3. Backtracking (Optimal) is same as striver's solution and what I have written
// here they used '#' but i used '\0'
// no need to save the character at r, c position for backtracking.. since we know that is == word.charAt(i) before we call next recurrsion calls, we can use wor.charAt(i) in backtracking step after recurrsion
// same time complexity O(m * 4^n)
// recurrsion stack space same O(n), no extra space since we modify and reuse the given board, O(1) for output
public class Solution {
    ...
    private boolean dfs(char[][] board, String word, int r, int c, int i) {
        ...
        if (r < 0 || c < 0 || r >= ROWS || c >= COLS ||
            board[r][c] != word.charAt(i)) { //  no need board[r][c] == '#' because # is not in our word. # always  board[r][c] != word.charAt(i))
            return false;
        }
