...
class Solution_bit_backtracking {
...
// upto ind-1 already fixed for permutation
// ind is the index we are fixing now
// ind+1 to till last. from this we will choose one for ind index
class Solution {
    private List<List<Integer>> ans;
    public List<List<Integer>> permute(int[] nums) {
        ans = new ArrayList<>();
    ...
    private void backtrack(int ind, int[] nums) {
        if(ind == nums.length) { // this index cannot be fixed now,. or use if(perms.size() == nums.length)
            ans.add(Arrays.stream(nums).boxed().collect(Collectors.toList())); // or use .collect(Collectors.toCollection(ArrayList::new)); for ArrayList specific
        ...
        // we need to use ind also so that ind+1 is called if ind is already nums.length-1, last index. then only base case recurrsion would be called
        for(int i = ind; i<nums.length; i++) {
            swap(nums, ind, i);
            backtrack(ind+1, nums);
            // we need to backtrack because if not, in next iteration we will swap the non fixed elements (ind+1 till end side) with another non fixed element in that side
            swap(nums, ind, i);
        }
    }

    private void swap(int[] nums, int i, int j) {
        int temp = nums[i]; // arrays will be swapped in place the elements
        nums[i] = nums[j];
        nums[j] = temp;
    }
}