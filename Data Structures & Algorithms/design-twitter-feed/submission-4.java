...
class Twitter {
    ...
    */
    ...
    Map<Integer, List<int[]>> userTweets; // values are list of pair of count and tweetId
    ...
    public List<Integer> getNewsFeed_bruteForce(int userId) {
    /*
    ...
    */
        ...
        // neededUsers.add(userId); // this will be added 2 times if user follow themselves
        if(userFollowees.containsKey(userId)) {
            neededUsers.addAll(userFollowees.get(userId));
            if( ! userFollowees.get(userId).contains(userId)) 
    ...
    public List<Integer> getNewsFeed(int userId) {
        PriorityQueue<int[]> lastestTweets = new PriorityQueue<>((a, b) -> b[0] - a[0]); // {count, tweetId, UserId, i} who posted it, i is index of 
        // userTweets for each user is asc order timestamp. so we can remove from last
        List<Integer> neededUsers = new ArrayList<>();
        // neededUsers.add(userId); // this will be added 2 times if user follow themselves
        if(userFollowees.containsKey(userId)) {
            neededUsers.addAll(userFollowees.get(userId));
        ...
        for(int user : neededUsers) { // O(n logn) , loop is n, inside insert is logn. 
            List<int[]> neededTweets = userTweets.get(user); // each element is array of {count, tweetId}
            if(neededTweets == null) continue; // this user not posted any tweets
            int i = neededTweets.size() - 1; // insert last tweet by the user
            if(i>=0) {
                int[] latestTweetOfUser = neededTweets.get(i);
                lastestTweets.offer(new int[]{latestTweetOfUser[0], latestTweetOfUser[1], user, i});
            }
        }

        List<Integer> ans = new ArrayList<>();
        while(ans.size() < 10) {
            if(lastestTweets.isEmpty()) break;

            int[] latestTweet = lastestTweets.poll();
            ans.add(latestTweet[1]);

            int i = latestTweet[3] - 1;
            if(i>=0) {
                int[] nextLatestTweetOfUser = userTweets.get(latestTweet[2]).get(i);
                lastestTweets.offer(new int[]{nextLatestTweetOfUser[0], nextLatestTweetOfUser[1], latestTweet[2], i});
            }
        }
        return ans;
    }
