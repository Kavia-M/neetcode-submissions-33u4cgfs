...
// Given solution as it is SORTING but using descending
class Solution {
    public int leastInterval(char[] tasks, int n) {
        Integer[] count = new Integer[26];
        Arrays.fill(count, 0);
        
        // Integer[] initializes elements to null, not 0.
        // So count[i]++ causes NullPointerException (null cannot be incremented).
        // Primitive int[] initializes elements to 0 automatically.
        
        for(char task : tasks) { // time O(m) m tasks, can be greater than 26 also
            count[task - 'A']++;
        }

        // to use reverse order we need Integer
        Arrays.sort(count, Comparator.reverseOrder()); // O(26 log 26) => constant => O(1)
        int maxf = count[0]; 
        int idle = n * (maxf - 1);
