...
// GIVEN Solution 2
// BFS update the rightNode in for loop, the last updated will be the right node that is size-1 index
class GIVEN_Solution_2 {
    public List<Integer> rightSideView(TreeNode root) {
        List<Integer> res = new ArrayList<>();
        Queue<TreeNode> q = new LinkedList<>(); // we will add null nodes also
        q.offer(root); // no null check
        ...
        while(!q.isEmpty()) {
            ...
            for(int i = 0; i<size; i++) {
                ...
                if(curr!=null) {
                    // NULL CHECK
                    // update right if curr is non null only
                    rightNode = curr; // if only a middle node in a level is non null only it is considered for ans
                    q.offer(curr.left);
                    q.offer(curr.right);
                } 
            }
            // outside for loop
            if(rightNode != null) { // if null means not even once (if) inside (for) is executed (we reached null level, below leaf level)
...
// right left. right comes 1st , modified BFS
// only non null in q
class Solution {
    public List<Integer> rightSideView(TreeNode root) {
        List<Integer> ans = new ArrayList<>();
        ...
        while(!q.isEmpty()) {
            // 1st one is right 
            // peek it not remove it
            ans.add(q.peek().val);

            int size = q.size(); // all elements including right
            for(int i=0; i < size; i++) {
                // if it is last one in this level add to answer
                TreeNode curr = q.poll(); // no need null check
                // add only non null children (right should come 1st)
                if(curr.right!=null) q.offer(curr.right);
                if(curr.left!=null) q.offer(curr.left);
            }
        }
