...
class Solution {
    public int maxDepth(TreeNode root) {
        ...
        return level; // the CONTINUE STMT is for 'for' loop only, for a null layer it enters the while loop and level++, so last lavel is null level, so correct level is level-1
    }
}
