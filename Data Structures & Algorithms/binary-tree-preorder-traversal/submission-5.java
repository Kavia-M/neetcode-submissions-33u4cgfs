...
// The given iterative solution
// this is same as ChatGPT given one
// instead of pushing left and popping immediately, we make left as curr so that it will be processed next
public class Solution {
    public List<Integer> preorderTraversal(TreeNode root) {
        List<Integer> res = new ArrayList<>();
        Stack<TreeNode> stack = new Stack<>(); // stack stores the future values to be processed
        TreeNode cur = root; // curr is root to be processed

        // curr is moved to left subtree till end
        // right nodes with null also saved in stack
        // once the popped right being null, the previous curr was a leaf node, else part is done again popping happend to go to right node ONE LEVEL ABOVE
        while (cur != null || !stack.isEmpty()) {
            if (cur != null) {
                res.add(cur.val); // add curr val first as it is preorder
                stack.push(cur.right); // save it for process in future since we loss the pointer to right node once we move to left substree
                cur = cur.left; // curr is completed move to left substree
            } else {
                cur = stack.pop(); // once all left is completed and curr becomes null we move to right which is stored in stack
            }
        }
