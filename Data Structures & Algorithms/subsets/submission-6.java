...
Limitation 1: Integer overflow
int = 32 bits
Safe shift range:
n ≤ 30   safe
n = 31   overflow (becomes negative), shift 31 bits means 32nd bit is set

New limits
Type	Max n (no overflow)
int	    30
long	62
...
// here we handle upto n=62. for int we can have upto n=30 only
class Solution {
    public List<List<Integer>> subsets(int[] nums) {
        int n = nums.length; // just for simplicity 
        List<List<Integer>> ans = new ArrayList<>();

        // MAKING IT LONG, use L in 1 
        for(long i=0; i<(1L<<n); i++) {
            
            List<Integer> subset = new ArrayList<>();
            
            // n bits only, n will be within limits no need to make it long
            for(int j = 0; j<n; j++) {
                // IMPORTANT: & with int and long, java promotes int to long
                // but (i & (1<<j)) != 0 ---- here 1<<j may have overflowed before the promotion happen, so promoted 1<<j will be -ve if overflow occurred
                // i<<j can be 2^n range. while j is in range of n only

                // for << type of the result depends on the left operand, so Make 1 as 1L
                if((i & (1L<<j)) != 0) {
                    subset.add(nums[j]);
                }
            }
