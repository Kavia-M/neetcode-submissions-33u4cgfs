...
class Solution_given_sol2_HashSet {
...
// Time complexity - O(n!)
// Auxilary space O(n) - n for recurrsion stack space. 2n for hashsets diagonals

// ----------------------------------------------------------------

/**
 * MATHEMATICAL DIAGONAL MAPPING (n x n board)
 * -------------------------------------------
 * 
 * 1. Positive Diagonals (Bottom-Left to Top-Right): [r + c]
 *    - Constant Property: The sum of row and column indices is constant.
 *    - Minimum Value:  0 + 0 = 0
 *    - Maximum Value:  (n-1) + (n-1) = 2n - 2
 *    - Distinct Values: 2n - 1 (e.g., for n=8, range is 0 to 14)
 *    - Array size:     2n (for safe indexing)
 *    - we can also use size 2n-1 [0....(2n-2)]
 *
 * 2. Negative Diagonals (Top-Left to Bottom-Right): [r - c + n]
 *    - Constant Property: The difference of row and column indices is constant.
 *    - Minimum Value:  0 - (n-1) = -(n-1) // 1st row last column
 *    - Maximum Value:  (n-1) - 0 = n-1
 *    - Range Shift:    We add 'n' to avoid negative array indices.
 *    - Shifted Min:    -(n-1) + n = 1
 *    - Shifted Max:    (n-1) + n = 2n - 1
 *    - Distinct Values: 2n - 1
 *    - Array size:     2n (for safe indexing)
 *    - we can also use shifting +(n-1) now array size will be 2n-1 [0....(2n-2)]
 *
 * SUMMARY:
 * Total cells = n^2
 * Total Rows/Cols = n
 * Total Diagonals (per direction) = 2n - 1
 */

class Solution_visitedArrays_my_own_1 {
    boolean[] col, posDiag, negDiag; // we cannot initialise here as we do not know size n 
    List<List<String>> ans = new ArrayList<>();
    char[][] board;
    ...
    public List<List<String>> solveNQueens(int n) {
        ...
        col = new boolean[n];
        // since we are using 2*n - 1 as size. we need to handle if n==0 case
        if(n==0) {
            ans.add(new ArrayList<>());
            return ans;
        }
        posDiag = new boolean[2*n - 1]; // * has more precedence
        negDiag = new boolean[2*n - 1];
        // all above 3 are filled with false by default
        board = new char[n][n];
    ...
    private void backtrack(int r) {
        ...
        for(int c=0; c<board[0].length; c++) {
            // WE CAN TRY with c-r also. we just need difference, r-c and c-r both works
            if(col[c] || posDiag[r+c] || negDiag[c-r + board.length -1])
                continue; // go and check next column
            ...
            // If 2 numbers increase at same rate, their difference is maintained. 
            // if one number increase, one decrease at same rate their sum is maintained
            posDiag[r+c] = true; // this is the secondary diagonal. here row increases column DECREASES. so that increased row and decreased column MATCH up with R+C
            negDiag[c-r + board.length -1] = true; // this is the MAIN PRIMARY diagonal. here row and column both increases. So the difference R-C between row can column is maintained. r+1 - c+1 == r-c
            ...
            col[c] = false;
            posDiag[r+c] = false;
            negDiag[c-r + board.length - 1] = false;
            // backtrack this cell state to dot .
            board[r][c] = '.';
        }
    }
...
// time complexity O(n!)
// auxilary space O(n) - O(n) for recurrsive stack space. O(n) for bool arrays

// -----------------------------------------------------------------------------------------

// Given solution 3. Backtracking (Visited Array)
class Solution_given_vistedArray {
    boolean[] col, posDiag, negDiag; // we cannot initialise here as we do not know size n 
    List<List<String>> ans = new ArrayList<>();
    char[][] board;
    ...
    public List<List<String>> solveNQueens(int n) {
        ...
        // using 2n as size to handle n==0 case
        // though this is not possible inside the backtrack function (as it will return at beginning when n==0)
        // if it tried to calculate r - c + n - 1.   if r, c, n are 0, we try to attempt -1 index which is not allwed in java
        // so for shifting also we do (+ n) only 
        posDiag = new boolean[2*n]; // only 0 to 2n-2 is used, last element is unused
        negDiag = new boolean[2*n]; // 1 to 2n-1 is used. 0th index is unused
        // all above 3 are filled with false by default
        board = new char[n][n];
...
// bit mask. upto n = 15, we can use integers to track, so that for diagonals 2n = 30
class Solution_Given_bitwise {
    long col = 0, posDiag = 0, negDiag = 0; // no col or row is true initially
    List<List<String>> ans = new ArrayList<>();
    char[][] board;
    ...
    private void backtrack(int r,  int n) {
        ...
        for(int c=0; c<n; c++) {
            // WE CAN TRY with c-r also. we just need difference, r-c and c-r both works
            if(checkBit(col, c) || checkBit(posDiag, r+c) || checkBit(negDiag, r-c + n))
                continue; // go and check next column
            
            // if not in any set, then we can add queen and recurse and backtrack

            // setting these bits we can use bitwise OR | ..... but since we already know this bits are not set. these are 0. exor with 1 will result in 1 only
            col ^= 1L << c; // ^= has less precedence
            posDiag ^= 1L << r+c; // + has more precedence 
            negDiag ^= 1L << r-c+n; // + - has more precedence and left to right associativity
            ...
            // backtrack using same XoR. XoR with 1 will simple toggle the bit. since the bit was set, it will eb unset
            col ^= 1L << c;
            posDiag ^= 1L << r+c;
            negDiag ^= 1L << r-c+n;
            // backtrack this cell state to dot .
    ...
    private boolean checkBit(long num, int i) {
        // () is important as > has more precedence than &
        return ((num & (1L << i)) > 0); // != 0 can also be used since we already avoided overflow by takign long
    }
}
// time complexity O(n!)
// space complexity - auxilary space O(n) for recurrsion stack space. O(1) for the int or long in bit manipulation. O(n^2) for the board

// -----------------------------------------------------------------------------------

// Bit manipulation
// since the given contraints say n<=8, so we can stick to int itself. no need long
// here instead of using xor to toggle a bit, we set it using | and unset it using & and ~ 
class Solution {
    int col = 0, posDiag = 0, negDiag = 0; // no col or row is true initially
    List<List<String>> ans = new ArrayList<>();
    char[][] board;
    ...
    private void backtrack(int r,  int n) {
        ...
        for(int c=0; c<n; c++) {
            // WE CAN TRY with c-r also. we just need difference, r-c and c-r both works
            // << has more precedence than & so ne need inner bracket, + - has more precedence than <<
            if((col & 1<<c) > 0 || (posDiag & 1<< c+r ) > 0 || (negDiag & 1<< c-r+n) > 0 )
                continue; // go and check next column
            
            // if not in any set, then we can add queen and recurse and backtrack

            col |= 1 << c; // |= has less precedence
            posDiag |= 1 << c+r; // + has more precedence 
            negDiag |= 1 << c-r+n; // + - has more precedence and left to right associativity
            ...
            // for unset, we INVERT the mask using bitwise NOT. it makes the mask, given bit as 0 and other bits as 1
            // & with 0 in given bit makes the given bit 0.   & with 1 in other bits keep the other bits as they are 
            col &= ~(1 << c); // ~ has more precedence so we need ()
            posDiag &= ~(1L << c+r);
            negDiag &= ~(1L << c-r+n);
            // backtrack this cell state to dot .
            board[r][c] = '.';
