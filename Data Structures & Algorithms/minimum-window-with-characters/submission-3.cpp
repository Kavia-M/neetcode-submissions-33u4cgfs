...
public:
    ...
    string minWindow(string s, string t) {
        ...
        for(i=0; i<=n1-n2; i++) {
            ...
            if(matches == 52) {
                update_start_end(start, end, i, j-1); // j-1 is used since this is outside loop
                cout<< start<< ' ' << end << endl;
            }
...
public:
    ...
    string minWindow(string s, string t) {
        ...
        if(matches == 52) {
            update_start_end(start, end, 0, n2-1);
        }
        // for safety in below loop check condition we check n2>n1 return

        // starting j is 1 greater than already seen j
        int i,j;
        for(i=0, j=n2; j<n1; j++) {
            ...
            cout<<"i " << i << " j " << j << " matches " << matches << endl;
            if(matches == 52) {
                while((i<=j) && ((s_freq[get_index(s[i])]-1) >= t_freq[get_index(s[i])])) {
                    s_freq[get_index(s[i])]--;
                    i++; // remove current i
                    // matches won't change
                }
                update_start_end(start, end, i, j);
            }
        }
