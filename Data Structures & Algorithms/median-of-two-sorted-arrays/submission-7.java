...
class Solution {
    ...
    */
    ...
    public double findMedianSortedArrays(int[] nums1, int[] nums2) {
        ...
        int half = (n1 + n2) / 2;
        int l = 0, r = n1 - 1;
        int x = 0; // start of right of medium
        int y = half - x;
        // // handling 0 lenght
        // if(n2 == 0) {
        //     // n1 <= n2, so n1 should also be 0
        //     return 0.0;
        // }
        // if(n1 == 0) {
        //     // loop will not run
        //     x = -1; // start of right array so x is out of bound x-1 also out of bound;
        ...
        if((n1+n2)%2 == 1) {
            ...
            else if(isIndexOutOfBound(y, n2)) {
                return a[x];
            }
        return Math.min(a[x], b[y]);
            ...
            int maxLeft = isIndexOutOfBound(x-1, n1) ? b[y-1] :
                            (isIndexOutOfBound(y-1, n2) ? a[x-1] : Math.max(a[x-1], b[y-1]));
            int minRight = isIndexOutOfBound(x, n1) ? b[y] :
                            (isIndexOutOfBound(y, n2) ? a[x] : Math.min(a[x], b[y]));
