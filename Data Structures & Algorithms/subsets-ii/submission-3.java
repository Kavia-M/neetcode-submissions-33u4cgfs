...
class Solution_backtracking_with_for_loop {
    List<List<Integer>> ans = new ArrayList<>();
    public List<List<Integer>> subsetsWithDup(int[] nums) {
        Arrays.sort(nums);
...
// time n* 2^n 
// extra space. O(n) recurrsion calls at a time. height of recurrsion tree.

// ----------------------------------------------------------------

// ITERATION
// prev always refer to the starting point of elements added in previous step
// so this will be size of ans before adding elements

// if it is a duplicate, we cannot take it if previous is not taken
class Solution {
    public List<List<Integer>> subsetsWithDup(int[] nums) {
        List<List<Integer>> ans = new ArrayList<>();
        Arrays.sort(nums);
        ...
        // if it is a duplicate then start from the prev. or start from 0
        for(int i = 0; i<nums.length; i++) {
            int start = (i!=0 && nums[i]==nums[i-1]) ? prev : 0;
            // before we add update previous
            int end = ans.size();
            prev = end;
            for(int j = start; j<end; j++) {
                // FOR SUBSET WE NO NEED TO ADD AT EACH INDEX, that is for permutations. here we add at last is enough. order inside each subset not matters
