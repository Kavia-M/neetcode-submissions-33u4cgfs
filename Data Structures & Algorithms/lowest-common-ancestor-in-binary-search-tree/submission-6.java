...
class Solution_iterative {
    ...
    public TreeNode lowestCommonAncestor(TreeNode root, TreeNode p, TreeNode q) {
        ...
        TreeNode curr = root;
        while(true) { // instead in solution it is taken as curr!=null
...
// GIVEN solution 2 safe case
class Solution {
    // ASSUMPTION, both nodes are in tree, if we get only one as final result, it means the other is a subtree under that node
    public TreeNode lowestCommonAncestor(TreeNode root, TreeNode p, TreeNode q) {
        if(p==null || q==null) // extra check
            return null;
        
        TreeNode curr = root;
        while(curr!=null) { // instead in solution it is taken as curr!=null
            if(Math.max(p.val, q.val) < curr.val)
                curr = curr.left;
            else if(Math.min(p.val, q.val) > curr.val)
                curr = curr.right;
        ...
        // curr == null, same for if root is null
        return null;
    }
}